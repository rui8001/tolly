...
The JSON contract contains per-tool `ranges`, `daily`, `projects`, and optional provider-authored `quota` metadata, plus `_pricing`, `_daily`, `_projects`, and optional `_errors` metadata. See [`../docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) and the unit tests for the contract.
